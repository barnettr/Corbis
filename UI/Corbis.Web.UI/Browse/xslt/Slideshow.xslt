  ...
  <xsl:template match="Slideshow">
		<xsl:if test="boolean(@Url)">
			<script type="text/javascript">				
				var swfUrl = '<xsl:value-of select="@Url"/>';
				...
				// Bug fix #18760 - cache buster for IE7
				var randomNumber = parseInt(Math.random()*99999999);
				if (Browser.Engine.trident == true) swfUrl += "?r=" + randomNumber;

				swfobject.embedSWF(swfUrl, 'FlashContent', '<xsl:value-of select="@Width"/>', '<xsl:value-of select="@Height"/>', "9.0.115.0", false, flashvars, params, attributes);
			</script>
		</xsl:if>
		<div class="Slideshow">
