...
s.linkInternalFilters="javascript:,corbis.com,localhost,samdevextui,samdevpro,samsqaextui,samsqapro"
...
function s_doPlugins(s) {
	...
	// Language for commerce
		s.eVar2=s.getValOnce(s.prop2,"prop2",0);
	// Account Segment for commerce
		s.eVar3=s.getValOnce(s.prop2,"prop3",0);
	// Market Segment for commerce
		s.eVar4=s.getValOnce(s.prop2,"prop4",0);
	// Industry Segment for commerce
		s.eVar5=s.getValOnce(s.prop2,"prop5",0);
	// Company Type for commerce
		s.eVar6=s.getValOnce(s.prop2,"prop6",0);	
	
    //  How to call the plugin in doPlugins:
		s.maxFlashVersion=9;
        s.prop15=s.detectFlash('s_fv');							
}
s.doPlugins=s_doPlugins
/*
 * maxFlashVersion should be one more than the current version of Flash
 * available. For example, right now Flash 8 is the highest version. If
 * You set this too high (e.g. 15), then it will take longer to detect
 * Flash.
 *
 * The parameter to the detectFlash function is the name of the session
 * cookie that will store the Flash version. This means that Flash will
 * only be detected on the first page of someone's visit, thus increasing
 * speed.
 *
 * WARNING: In IE, the Flash Detection plugin uses VB Script to create an
 * instance of a Flash object for each version of Flash, starting from
 * maxFlashVersion and going down. This process has been known to take up
 * to 3 seconds in some cases.
 */
...
+"'')+'://'+(s.ssl?'smetrics':'metrics')+'.corbis.com/b/ss/'+s.un+'/1/H"
